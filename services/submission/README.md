...
│   ├── queue_utils.py     # Celery configuration & helper
...
1. **"Could not read from input file: Is a directory"**  
   - **Solution**: Ensure Compose file mounts `init.sql` as a file, not directory:
     ```yaml
     volumes:
...
2. **`OperationalError: could not translate host name "db" to address`**  
   - **Solution**: Ensure `depends_on: [db]` is set for services and retry after few seconds

3. **Celery worker running as root**  
...
4. **"Problems database not available"**  
   - **Solution**: Verify `problems-db` service is running on port 5433 and `init.sql` contains `CREATE TABLE problems`
